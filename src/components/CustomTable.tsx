...
const CustomTable = <T = any,>(props: CustomTableProps<T>) => {
  ...
  const onDelete = useCallback(() => {
    ...
    setDataSourceState(
      dataSource.filter(
        (item: any) => !selectedRowKeys.includes(item[rowKeyPattern]),
      ),
    );
      ...
      const res = dataSource.filter((item: any) => {
        return searchKeys?.some(key => {
          const val = key.includes('.')
            ? fromDotKeyToObjectValue(item, key)
            : item[key];
  ...
  const onSortChange = useCallback(
    (_sortDescriptor: SortDescriptor) => {
      ...
      const sortedData = [...dataSource].sort((a: any, b: any) => {
        const sortKeyA = fromDotKeyToObjectValue(
          a,
          _sortDescriptor.column as string,
        );
        const sortKeyB = fromDotKeyToObjectValue(
          b,
          _sortDescriptor.column as string,
        );
    ...
    return (
      <div className="flex space-x-2">
        ...
        {dataSource.length > 0 && (
          <CSVLink
            filename={
              csvFileName?.includes('.csv') ? csvFileName : csvFileName + '.csv'
            }
